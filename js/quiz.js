...
// Rule: it's not possible to move to the next question without answering.
// ============================================================================
...
if (quizUser) {
  ...
  let current = 0;
  let maxReached = 0; // furthest index already reached (answered)
  ...
  function renderQuestion() {
    ...
    document.getElementById('question-mount').innerHTML = `
      ...
      ${!selected ? `<div class="answer-hint">Pick an answer to unlock the next question.</div>` : ''}`;
  ...
  function renderDots() {
    document.getElementById('dots-mount').innerHTML = questions.map((q, i) => {
      ...
      const locked = i > maxReached + 1 && !answered; // can't skip beyond the next unlockable question
  ...
  document.getElementById('btn-next').addEventListener('click', () => {
    if (!answers[questions[current].id]) return; // safety: no answer, no moving on
  ...
  document.getElementById('btn-submit').addEventListener('click', () => {
    ...
    if (unanswered > 0) {
      const proceed = confirm(`You still have ${unanswered} unanswered question(s). Finish anyway?`);
      if (!proceed) return;
    }
    finishExam();
  });
