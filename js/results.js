...
if (resultsUser) {
  ...
  if (!attempt || attempt.username !== resultsUser.username) {
    ...
    reviewMount.innerHTML = attempt.answers.map((a, idx) => {
      ...
      if (!q) return '';
      // N'affiche que ce qui compte : si c'est correct, seulement la bonne
      // réponse ; si c'est faux, le choix fait + la bonne réponse.
      const lettersToShow = a.correct
        ? [a.correctAnswer]
        : [...new Set([a.chosen, a.correctAnswer].filter(Boolean))];
      return `
