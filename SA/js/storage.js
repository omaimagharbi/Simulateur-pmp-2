...
// This is a 100% static project (no server). All data — accounts, attempts,
// questions added by the admin — lives in the browser of the person using
// the site. If you want a real backend shared across multiple devices,
// you'll need to wire an API behind these same functions (see README.md).
// ============================================================================

const DB_KEYS = {
  users: 'pmp_users',
  session: 'pmp_session',
  customQuestions: 'pmp_custom_questions', // admin additions/edits, indexed by id
...
// --- password hash: simple, client-side. This is NOT real cryptographic
// security — it only avoids storing the password in plain text in
// localStorage. Do not reuse this scheme for a real product.
...
const Store = {
  ...
  createUser(username, password) {
    ...
    if (users[key]) return { ok: false, error: 'This username already exists.' };
  ...
  // Merges QUESTIONS (data.js) + admin overrides + added questions,
  // minus deletions.
  getAllQuestions() {
    ...
    // questions added by the admin (negative id or > base max)
